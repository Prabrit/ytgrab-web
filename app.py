...
class _JobLogger:
    """Captures yt-dlp's verbose/debug output for one job. On success this
    is just discarded, but on failure it gets printed to the server log
    (Render's Logs tab) — the final error line alone often isn't enough to
    tell what actually happened (which client, whether a PO token was used,
    what formats were found), so this keeps the full trace instead of
    losing it."""
    def __init__(self):
        self.lines = []
...
def run_job(job_id, url, audio_only, audio_format, quality):
    ...
    opts = {
        ...
        "ignoreerrors": False,
        "quiet": True,
        "verbose": True,      # paired with logger= below: full detail is
        "logger": job_logger,  # captured instead of printed directly
        ...
        # Per yt-dlp's own PO Token Guide (github.com/yt-dlp/yt-dlp/wiki/PO-Token-Guide,
        # checked 2026-08-19): "tv" needs NO PO token at all (just cookies,
        # which we already have, to avoid DRM'd formats) - the most reliable
        # option. "mweb" is their officially recommended client to pair with
        # a PO token provider for GVS tokens, as a second source of formats.
        # android/web both require GVS tokens we weren't reliably getting -
        # dropped in favor of these two.
        "extractor_args": {
    ...
    # Print the full trail to stdout (Render's Logs tab) so the actual
    # cause is visible, not just the final exception message.
    print(f"--- yt-dlp trace for failed job {job_id} ({url}) ---")
    for line in job_logger.lines:
        print(line)
    print(f"--- end trace for {job_id} ---")

    # Pull out the lines that actually explain *why* — PO token / format /
    # client details — and surface those directly in the job's error field,
    # so the reason is visible on the page itself, not just in server logs.
    keywords = ("pot", "sabr", "format", "client", "skip", "token")
    relevant = [
        ln for ln in job_logger.lines
        if any(k in ln.lower() for k in keywords) and len(ln) < 300 and "\n" not in ln
    ]
    detail = " | ".join(relevant[-5:])  # last few relevant lines, most recent first context
    error_msg = str(last_exc)
    if detail:
        error_msg = f"{error_msg}\n\nDiagnostic trail: {detail}"
